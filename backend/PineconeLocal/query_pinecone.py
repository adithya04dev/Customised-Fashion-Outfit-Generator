...
import os
import random
from PineconeLocal.pinecone_utils import setup_pinecone
...
def perform_query(pinecone_index, bm25, model, query, hard_filters, top_k=5, alpha=0.05):
    # setting men as gender
    hard_filters['gender'] = {"$in": ["unisex","men"]}
    print("before encoding")
    try:
        print("query in perform query:",query)
        sparse = bm25.encode_queries(query)
        dense = model.encode(query).tolist()
    except Exception as e:
        print("Exception occurred while encoding query:", e)
        return {}
    # scale sparse and dense vectors
    hdense, hsparse = hybrid_scale(dense, sparse, alpha)
    # search
    print("before pinecone index query.")
    print(hard_filters)
    try:
        ...
        print("Exception occurred while querying Pinecone:", e)
        return {}
    return result
...
def query_pinecone(query, pinecone_index, model, bm25, hard_filters):
    top_k = 2
    print("just before quering pinecone")

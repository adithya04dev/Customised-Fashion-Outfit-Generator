...
from pinecone_utils import setup_pinecone
import pickle
import chardet
import time
import os
...
def insert_data_parallel(pinecone_index, model, bm25, data, batch_size=500, num_threads=20):
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for batch_idx, i in enumerate(range(0, len(data), batch_size)):
                ...
                img_batch = []
                for idx,x in enumerate(meta_dict): 
                    currImageId=x["id"]
                    currImageName=rf'C:\Users\adith\Documents\Projects\python-projects\genai_hackathon\lookify\backend\downloaded_images\{currImageId}.jpg'
                ...
                #     print(type(x))
                #     print(x)
                print(f"length of batches : {len(pinecone_query_string)}")
                print(f"length of images : {len(img_batch)}")
...
def upsert_csv(csv_file, char_enc, pinecone_index, model, bm25):
    try:
        ...
        insert_data_parallel(pinecone_index, model, bm25, data, batch_size=500, num_threads=20)
...
def main():
    ...
    startTime=time.time()
    csv_file = r'C:\Users\adith\Documents\Projects\python-projects\genai_hackathon\lookify\backend\dataset\main_dataset.csv'
    ...
    endTime=time.time()
    print('total time to upsert',endTime-startTime)
